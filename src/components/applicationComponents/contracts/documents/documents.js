...
import ContractDocument from '../../../documents/documents';
...
class ContractDocuments extends Component {
    ...
    componentDidMount() {
        this.props.actions.FetchMasterContractDocumentTypes();
        this.props.actions.FetchContractDocuments();
        const elems = document.querySelectorAll('.datepicker');
        const instances = MaterializeComponent.Datepicker.init(elems);
    ...
    documentSubmitHandler = (e) => {
        ...
        if (this.props.showButton === true) {
            if (this.updatedData.documentType === "") {
                ...
                if (this.props.editContractDocumentDetails.recordStatus !== "N") {
        ...
        if (this.props.showButton === false) {
            if (this.updatedData.documentType === undefined || this.updatedData.documentType === "") {
                ...
                if (documentFile.length > 0) {
                    ...
                    const outOfCompanyVisible = document.getElementById("isOutOfCompanyVisible").checked;
                            ...
                            .then(response => {
                                filesToBeUpload.map((document, i) => {
                                    this.updatedData = {
                                        ...
                                        isOutOfCompanyVisible: outOfCompanyVisible,
                                        recordStatus: "N",
                                        contractDocumentId: Math.floor(Math.random() * (Math.pow(10, 5))),
    ...
    pasteRecord = () => {
        ...
        if (recordToPaste.length > 0) {
            ...
            this.props.actions.PasteContractDocumentUploadData({ Id: recordToBeCopy })
                .then(response => {
                    if (response) {
                        recordToPaste.map((record, i) => {
                            this.updatedData = {
                                ...
                                isOutOfCompanyVisible: record.isOutOfCompanyVisible,
                                recordStatus: "N",
                                contractDocumentId: Math.floor(Math.random() * (Math.pow(10, 5))),
                                uploadDataId: response[i],
                                modifiedBy: this.props.loggedInUser
    ...
    render() {
        ...
        const { masterContractDocumentTypesData, showButton, editContractDocumentDetails, DocumentsData } = this.props;
        let contractDocumentsData = [];
        if (DocumentsData) {
            contractDocumentsData = DocumentsData.filter(document => document.recordStatus !== 'D');
        }
        this.props.editContractDocumentDetails.isVisibleToCustomer && (document.getElementById("isVisibleToCustomer").checked = (editContractDocumentDetails.isVisibleToCustomer === true ? true : false));
        this.props.editContractDocumentDetails.isVisibleToTS && (document.getElementById("isVisibleToTS").checked = (editContractDocumentDetails.isVisibleToTS === true ? true : false));
        this.props.editContractDocumentDetails.isOutOfCompanyVisible && (document.getElementById("isOutOfCompanyVisible").checked = (editContractDocumentDetails.isOutOfCompanyVisible === true ? true : false));
        ...
        return (
            <Fragment>
                ...
                <div className="customerBlock">
                                ...
                                <div className="row">
                                    <div className="col s6 mb-2">
                                        <CustomInput
                                            ...
                                            optionsList={masterContractDocumentTypesData}
                                            optionName='name'
                                            optionValue="name"
                                            onSelectChange={this.handlerChange}
                                            name="documentType"
                                            id="documentType"
                                            defaultValue={editContractDocumentDetails.documentType}
                                        />
                                    </div>
                                    {showButton ?
                                        <div className="col s12">
                                            <label className="customLabel">Document Name</label>
                                            <input type="text" id="documentName" name="name" defaultValue={editContractDocumentDetails.name} onChange={this.handlerChange} className="customInputs browser-default validate" disabled />
                                        </div> :
                                        <div className="col s12">
                                ...
                                <div className="row">
                                    <div className="col s4 m4">
                                        <CustomInput
                                            ...
                                            defaultChecked={editContractDocumentDetails.isVisibleToCustomer}
                                        />
                                    </div>
                                    <div className="col s4 m4">
                                        <CustomInput
                                            ...
                                            id="isVisibleToTS"
                                            onChangeToggle={this.switchHandlerChange}
                                            defaultChecked={editContractDocumentDetails.isVisibleToTS}
                                        />
                                    </div>
                                    <div className="col s4 m4">
                                        <CustomInput
                                            ...
                                            isSwitchLabel={true}
                                            switchName="isOutOfCompanyVisible"
                                            id="isOutOfCompanyVisible"
                                            onChangeToggle={this.switchHandlerChange}
                                            defaultChecked={editContractDocumentDetails.isOutOfCompanyVisible}
                                        />
                                    </div>
                                </div>
                    ...
                    <ContractDocument DocumentsData={contractDocumentsData}
...
ContractDocuments.propTypes = {
    ...
    masterContractDocumentTypesData: PropTypes.array.isRequired,
    gridProps: PropTypes.array.isRequired
};

ContractDocuments.defaultprops = {
    DocumentsData: [],
    masterContractDocumentTypesData: [],
    gridProps: {}
